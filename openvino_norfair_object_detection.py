...
def build_argparser():
    ...
    # args.add_argument('-m', '--model', default='./openvino/public/yolo-v4-tf/FP32/yolo-v4-tf.xml', required=False,
    #                   help='Required. Path to an .xml file with a trained model '
    #                        'or address of model inference service if using ovms adapter.')
    # available_model_wrappers = [name.lower() for name in DetectionModel.available_wrappers()]
    # args.add_argument('-at', '--architecture_type', default='yolov4', help='Required. Specify model\' architecture type.',
    ...
    # args.add_argument('-i', '--input', default='lp.mp4', required=False,
    ...
    common_model_args.add_argument('--labels', default='./openvino/public/yolo-v4-tf/FP32/obj.names', help='Optional. Labels mapping file.', type=str)
...
def main():
    ...
    cap = open_images_capture('lp.mp4', args.loop)

    if args.adapter == 'openvino':
        plugin_config = get_user_config(args.device, args.num_streams, args.num_threads)
        model_adapter = OpenvinoAdapter(create_core(), 'yolo-v4-tf.xml', device=args.device, plugin_config=plugin_config,
                                        max_num_requests=args.num_infer_requests, model_parameters = {'input_layouts': args.layout})
    elif args.adapter == 'ovms':
        model_adapter = OVMSAdapter('yolo-v4-tf.xml')
    ...
    model = DetectionModel.create_model('yolov4', model_adapter, configuration)
    ...
    while True:
        ...
        if results:
            ...
            for detection in objects:
                ...
                boxis = [xmin, ymin, xmax, ymax]
                boxes.append([xmin, ymin, xmax, ymax])
                confidence.append(detection.score)

                color = palette[class_id]
                
                classes.append(det_label)

                d = Detection(get_centroid(boxis, frame.shape[0], frame.shape[1]))
                d.data = boxis
                d.label = det_label

                total_detections.append(d)
            tracked_objects = tracker.update(detections=total_detections)
    
            draw_tracked_objects(frame, tracked_objects,2,(255,255,255),1,2,True,False,False)
